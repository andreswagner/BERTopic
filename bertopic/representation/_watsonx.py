...
class Watsonx(BaseRepresentation):
    """ Using the Watsonx API to generate topic labels based
    on one of their Foundation Models.

    The default method is `Instruct`.

    For an overview see:
    https://www.ibm.com/products/watsonx-ai/foundation-models

    Arguments:
        client: A `ibm_watsonx_ai.foundation_models` model
        ...
        exponential_backoff: Always default to False in this version
        chat: Always default to False in this version
    ...
    Then, get yourself an API key and use Watsonx's API as follows:

    ```python
    from bertopic import BERTopic
    from bertopic.representation import Watsonx
    from ibm_watsonx_ai.foundation_models import Model
    from ibm_watsonx_ai.metanames import GenTextParamsMetaNames as GenParams

    # Create your Watsonx client
    generate_params = {
        GenParams.MAX_NEW_TOKENS: 100,
    }
    watsonx_client = Model(
        model_id='meta-llama/llama-3-70b-instruct', #'mistralai/mixtral-8x7b-instruct-v01',
    ...
    representation_model= Watsonx(client=watsonx_client)
    ...
    representation_model = Watsonx(client, prompt=prompt, delay_in_seconds=5)
    ```
    ...
    def __init__(self,
        ...
        # Chat not implemented in this version, always changes as default to False
        if chat:
            chat=False
        if prompt is None:
        ...
        # Exponential backoff not implemented in this version, always changes as default to False
        ...
        self.prompts_ = []
        if self.client.params.get("stop_sequences"):
            self.stop_sequences = self.client.params.get("stop_sequences")
        else:
            self.stop_sequences =['###']
    ...
    def extract_topics(self,
        ...
        # Generate using Watsonx Language Model
        updated_topics = {}
        for topic, docs in tqdm(repr_docs_mappings.items(), disable=not topic_model.verbose):
            ...
            if self.chat:
                ...
                for ss in self.stop_sequences:
                    label = response.replace(ss, '').strip()
